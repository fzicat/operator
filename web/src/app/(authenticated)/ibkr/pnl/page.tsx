...
type PnLRow = Position & { totalPnl: number };

export default function IBKRPnLPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const withTotal: PnLRow[] = positionsData.map((p) => ({
        ...p,
        totalPnl: p.stockPnl + p.callPnl + p.putPnl,
      }));
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (p: PnLRow) => <NumericCell value={p.mtm} format="currency" />,
    },
    {
      key: "stockPnl",
      header: "Stk PnL",
    ...
    },
    {
      ...
      align: "right" as const,
      render: (p: PnLRow) => (
        <NumericCell value={p.totalPnl} format="currency" colorCode />
      ),
    },
  ];
